...
<el-card
  class="box-card"
  style="width:250px;padding: 0 3px 20px;margin-top:10px;position:fixed;z-index:9999"
  v-show="showSeason"
>
...
<input type="hidden" :value="value" :id="componentId">
</el-form>
</template>
<script>
export default {
  ...
  created() {
    if (this.defaultValue) {
      ...
      this.value = `${this.year}${arrAll.indexOf(str) + 1}`
    }
  },
  watch: {
  ...
  methods: {
    ...
    selectSeason(i) {
      ...
      this.value = `${this.year}${this.season}`
    }
  }
}
</script>